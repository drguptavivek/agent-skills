...
If full visual QA is explicitly requested, use the host agent's document-rendering workflow. Do not hardcode a versioned plugin path in a portable installation. On the verified development Mac, locate the current renderer dynamically and use the dedicated render venv so `pdf2image` is available:

```bash
PATH=~/.local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin \
TMPDIR=/private/tmp \
~/.local/venvs/docx-render/bin/python \
"$(find ~/.codex/plugins/cache/openai-primary-runtime/documents -path '*/skills/documents/render_docx.py' -print -quit)" \
file.docx \
